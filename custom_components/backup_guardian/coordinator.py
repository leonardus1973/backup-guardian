...
class BackupGuardianCoordinator(DataUpdateCoordinator):
    """Gestisce il recupero dati tramite le API di sistema."""
    ...
    async def _async_update_data(self):
        try:
            # USA SOLO LE API DI SISTEMA - NESSUN PERCORSO FISICO
            ...
            _LOGGER.error(f"Errore API Backup: {err}")
            raise UpdateFailed(f"Errore: {err}")
